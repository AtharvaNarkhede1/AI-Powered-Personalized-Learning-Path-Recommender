...
from app.models.schemas import ProfileOnboardingRequest
PASS, FAIL = ('PASS', 'FAIL')
results = []
...
GOALS = {'machine learning': ('machine learning', 'pytorch', 'deep learning', 'scikit', 'mlops', 'llm', 'linear algebra', 'probability', 'statistics', 'python'), 'robotics and ros': ('ros', 'robot', 'kinematics', 'control system', 'computer vision', 'opencv', 'embedded', 'microcontroller', 'c++', 'cad', 'linear algebra'), 'vlsi chip design verilog': ('verilog', 'vlsi', 'fpga', 'rtl', 'systemverilog', 'uvm', 'digital logic', 'timing analysis', 'computer architecture', 'semiconductor'), 'cloud devops kubernetes': ('kubernetes', 'docker', 'cloud', 'devops', 'terraform', 'linux', 'ci/cd', 'aws', 'infrastructure', 'pipeline automation'), 'structural civil engineering': ('structural', 'civil', 'concrete', 'etabs', 'staad', 'bim', 'revit', 'seismic', 'mechanics of materials', 'steel'), 'cybersecurity pen testing': ('security', 'pen test', 'cryptograph', 'network', 'linux', 'offensive', 'burp', 'nmap', 'penetration'), 'full stack web react': ('react', 'javascript', 'typescript', 'front-end', 'web', 'node', 'rest', 'graphql', 'api', 'database', 'sql & nosql'), 'data engineering pipelines': ('etl', 'elt', 'data modeling', 'spark', 'sql & data', 'distributed data', 'warehouse', 'airflow', 'dbt', 'data pipeline')}
precisions = []
for goal, needles in GOALS.items():
    ...
    hits = sum((1 for r in res if any((n in (r['track'] + ' ' + ' '.join(r['skills_covered']) + ' ' + r['branch']).lower() for n in needles))))
    prec = hits / max(1, len(res))
    precisions.append(prec)
    check(f"goal '{goal}' -> >=65% on-topic", prec >= 0.65, f'{hits}/{len(res)} ({prec:.0%})')
check('mean precision >= 0.85', sum(precisions) / len(precisions) >= 0.85, f'{sum(precisions) / len(precisions):.0%}')
...
check('downvote lowers the driving factor weight', ok, f'quality {before['quality']:.3f} -> {(added[-1].weights['quality'] if added else 0):.3f}')
print()
