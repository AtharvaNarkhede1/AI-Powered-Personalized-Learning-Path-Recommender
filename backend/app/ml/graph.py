"""Prerequisite DAG over course "rungs" = (branch, track, tier).

Edges come from `prerequisite_course_title`, resolved to a rung by title
(within the same track first, then anywhere in the catalog). Unresolved
prerequisites fall back to the tier below in the same track. Cycles are
broken by dropping the highest-tier edge. Depth = longest path from a root.
"""
...
def build_graph(catalog: Catalog) -> PrereqGraph:
    ...
    # title -> rungs (a title belongs to exactly one (branch, track, tier) in this dataset)
    ...
    for rung, positions in catalog.variant_index.items():
        ...
        for p in positions:
            ...
            # prefer a target in the same track
    ...
    # also chain tiers within a track so a bare ladder is always ordered
    ...
    # break cycles: drop the edge whose head is the highest tier
