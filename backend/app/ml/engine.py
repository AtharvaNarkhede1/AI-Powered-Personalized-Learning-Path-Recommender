...
from app.models.schemas import LearningPathResponse, Milestone, NextRecommendedAction, ResourceItem
...
class Engine:
    ...
    def warm(self) -> None:
        ...
        self._ready = True
        print(f'[ml.engine] warm complete in {time.time() - t0:.2f}s ({len(self.catalog)} courses, {len(self.graph.g)} rungs, {len(self.graph.unresolved)} unresolved prereqs)')
    ...
    def interpret_profile(self, profile, career_id: Optional[str]=None, gap_names: Optional[List[str]]=None) -> str:
    ...
    def _context(self, profile, career_id, db=None, profile_id=None):
        ...
        ctx = RankingContext(goal_sims=goal_sims, gap_terms=gap_terms, gap_sims=gap_sims, target_tier=target_tier, preferred_branches=preferred, home_branch=home_branch, career_branches=career_branches, weekly_hours=max(3, int(getattr(profile, 'hours_per_week', 10) or 10)), preferred_format=(getattr(profile, 'preferred_format', '') or '').lower(), satisfied_rungs=set(), weights=weights, affinities=affinities)
        return (ctx, goal_vec, goal_text, gap, gap_names)

    def _candidate_pool(self, ctx: RankingContext, career_id: Optional[str], goal_vec: Optional[np.ndarray]=None) -> List[int]:
        cat = self.catalog
        pool: set = set()
        if career_id:
            from app.data.taxonomy_data import CAREERS_DATABASE
        ...
        if goal_vec is not None and self.planner is not None:
            tsim = sorted(((float(np.dot(cen, goal_vec)), key) for key, cen in self.planner._track_centroids.items()), key=lambda x: -x[0])[:18]
            for _, key in tsim:
    ...
    def recommend(self, db, profile, goal_text: Optional[str]=None, career_id: Optional[str]=None, limit: int=12, exclude_planned: bool=False, profile_id=None) -> dict:
        self._require()
        ...
        if final:
            ...
            final = [sc for i, sc in enumerate(final) if i < keep_min or (ctx.goal_sims[sc.pos] >= 0.3 * best_sim and sc.score >= 0.52 * best_score)]
    ...
    def _planned_positions(self, db, profile_id) -> set:
        try:
            ...
            rows = db.query(MilestoneDB).join(LearningPathDB).filter(LearningPathDB.profile_id == profile_id).all()
            out = set()
            for m in rows:
    ...
    def _resource_item(self, pos: int, sc=None) -> ResourceItem:
        ...
        item = ResourceItem(id=cid, course_id=cid, title=str(row['course_title']), type='course', provider=str(row['provider']), url=f'https://www.google.com/search?q=' + str(row['course_title']).replace(' ', '+') + '+' + str(row['provider']).replace(' ', '+'), duration_hours=float(row['estimated_hours']), difficulty=str(row['difficulty_level']).lower(), skills_covered=skills[:8], rating=float(row['rating']), num_reviews=int(row['num_reviews']), is_free=str(row['provider']) in ('NPTEL', 'MITx', 'edX'), track=str(row['track']), branch=str(row['branch']))
        if sc is not None:
    ...
    def build_path(self, db, profile, career_id: str, profile_id=None) -> LearningPathResponse:
        ...
        plan: LearningPlan = self.planner.build_plan(ctx, goal_vec, weekly, timeline_weeks, ctx.target_tier, must_tracks=must_tracks)
        ...
        next_action = NextRecommendedAction(action_type='start_course', title=f"Start '{first.title}'" if first else 'Begin your first milestone', description=f'Kick off {milestones[0].title} to build momentum.' if milestones else 'Complete onboarding to generate your path.', milestone_id=milestones[0].id if milestones else 'ms_1', resource_id=first.id if first else None, estimated_minutes=45, urgency='high')
        return LearningPathResponse(id=f'path_{career['career_id']}', career_id=career['career_id'], career_title=career['title'], job_readiness_score=readiness['score'], estimated_total_hours=readiness['hours'], estimated_weeks=readiness['weeks'], hours_per_week=weekly, milestones=milestones, next_action=next_action, what_not_to_do_warnings=warnings, track_names=plan.tracks)
    ...
    def _plan_to_milestones(self, plan: LearningPlan, weekly: int) -> List[Milestone]:
        ...
        for seq, (phase, part, items) in enumerate(groups, start=1):
            ...
            yt_terms = [s for s in skill_names if s in real_skill_names][:3] or [it.rung[1] for it in items[:3]]
            ...
            if part:
                ttl = f'{ttl} (Part {part})'
            milestones.append(Milestone(id=f'ms_{seq}', sequence_order=seq, title=ttl, description=PHASE_DESC.get(phase, 'Work through these courses in order.'), estimated_hours=est_hours, estimated_weeks=max(1, round(est_hours / max(1, weekly))), status='in_progress' if seq == 1 else 'not_started', target_skills=skill_names, resources=resources, project={'title': f'{PHASE_TITLES.get(phase, 'Milestone').split(':')[-1].strip()} Project', 'description': f'Build a portfolio project using {', '.join(skill_names[:3])}.', 'required_deliverable': 'GitHub repo link + short demo video'}, assessment=quiz, youtube_extras=yt))
        return milestones
        ...
        for name in skill_names:
            for r in get_dynamic_youtube_resources(name)[:per_skill]:
                if r['id'] in seen:
                    continue
                seen.add(r['id'])
                out.append(ResourceItem(id=r['id'], title=r['title'], type='video', provider=r['provider'], url=r['url'], duration_hours=float(r.get('duration_hours', 1.0)), difficulty=r.get('difficulty', 'beginner'), skills_covered=r.get('skills_covered', [name]), rating=float(r.get('rating', 4.0)), is_free=True, match_reason=r.get('match_reason', f'YouTube pick for {name}')))
        return out
    ...
    def record_feedback(self, db, profile_id: Optional[str], event_type: str, course_id: Optional[str]=None, factors: Optional[dict]=None) -> dict:
        self._require()
    ...
    def model_snapshot(self, db, profile_id) -> dict:
        weights, affinities, row = self._learner_model(db, profile_id)
        return {'weights': [{'factor': f, 'weight': round(weights[f], 3), 'default': DEFAULT_WEIGHTS[f], 'delta': round(weights[f] - DEFAULT_WEIGHTS[f], 3)} for f in FACTORS], 'affinities': [{'key': k, 'value': round(v, 3)} for k, v in affinities.items()], 'update_count': row.update_count if row else 0, 'personalised': bool(row)}
engine = Engine()
