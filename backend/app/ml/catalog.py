"""Course catalog: load courses.csv into a pandas DataFrame plus the indices
the ranker / planner / graph need. Built once per process (cached by engine).
"""
...
Rung = Tuple[str, str, int]  # (branch, track, tier)
...
class Catalog:
    df: pd.DataFrame
    docs: List[str]                                  # field-weighted pseudo-doc per row
    quality: np.ndarray                              # Bayesian-shrunk rating in [0, 1]
    tiers: np.ndarray                                # int tier per row
    skill_lists: List[List[str]]                     # parsed skills_taught per row
    career_lists: List[List[str]]
    variant_index: Dict[Rung, List[int]] = field(default_factory=dict)   # rung -> row positions
    track_index: Dict[Tuple[str, str], List[int]] = field(default_factory=dict)
    title_index: Dict[str, List[int]] = field(default_factory=dict)      # lower title -> positions
    career_index: Dict[str, List[int]] = field(default_factory=dict)     # career (lower) -> positions
...
FIELD_WEIGHTS = {
    "course_title": 3, "track": 3, "skills_taught": 2, "career_paths": 2,
    "tools_covered": 1, "branch": 1, "industry_sectors": 1, "description": 1,
}
...
def load_catalog(csv_path: str) -> Catalog:
    if not os.path.exists(csv_path):
        raise FileNotFoundError(
            f"course dataset not found: {csv_path}\n"
            f"run  python -m scripts.generate_dataset  first."
        )
    ...
    # Bayesian-shrunk quality toward the global mean rating
    ...
    quality = np.clip((shrunk - 3.0) / 2.0, 0.0, 1.0)   # map ~[3,5] -> [0,1]

    skill_lists = [_split(c) for c in df["skills_taught"]]
    career_lists = [_split(c) for c in df["career_paths"]]

    cat = Catalog(
        df=df, docs=docs, quality=quality, tiers=tiers,
        skill_lists=skill_lists, career_lists=career_lists,
    )
