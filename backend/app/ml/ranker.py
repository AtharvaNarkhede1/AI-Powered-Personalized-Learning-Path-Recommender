    ...
    def rank(self, ctx: RankingContext, positions: List[int], limit: int = 12,
        ...
        for p in positions:
            ...
            fv = {
                ...
                "effort_fit": float(np.clip(1.0 - (row["estimated_hours"] - 3 * ctx.weekly_hours) / max(1.0, 3 * ctx.weekly_hours), 0.0, 1.0)),
                "format_pref": 1.0 if ctx.preferred_format and ctx.preferred_format.split("-")[0] in str(row["format"]).lower() else 0.5,
            }
