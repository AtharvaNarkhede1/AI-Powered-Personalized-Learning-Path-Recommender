...
PHASE_TITLES = {0: 'Phase 1: Foundations & Core Tools', 1: 'Phase 2: Applied Engineering & Systems', 2: 'Phase 3: Advanced Specialisation', 3: 'Phase 4: Industry Capstone & Job Readiness'}
PHASE_DESC = {0: 'Build the base vocabulary, math, and tooling every later step assumes.', 1: 'Apply the fundamentals to real components and workflows.', 2: 'Go deep on the specialised skills that define the role.', 3: 'Ship an end-to-end portfolio project and close remaining gaps.'}
...
class Planner:
    ...
    def _pick_tracks(self, ctx: RankingContext, goal_vec: np.ndarray, weekly: int, timeline_weeks: int, must_tracks: List[str] | None=None) -> List[tuple]:
    ...
    def build_plan(self, ctx: RankingContext, goal_vec: np.ndarray, weekly: int, timeline_weeks: int, target_tier: int, must_tracks=None) -> LearningPlan:
        ...
        ordered = sorted(chosen.items(), key=lambda kv: (self.graph.depth.get(kv[0], 0), kv[0][2], -kv[1].score))
        ...
        for rung, sc in ordered:
            ...
            info = explain(sc, row)
            unlocks = [self.cat.df.iloc[chosen[succ].pos]['course_title'] for succ in self.graph.g.successors(rung) if succ in chosen] if rung in self.graph.g else []
            prereq_titles = [self.cat.df.iloc[chosen[pre].pos]['course_title'] for pre in self.graph.prereq_rungs(rung) if pre in chosen]
            if prereq_titles:
                why = f'Take this after {prereq_titles[0]}. {info['headline']}'
            else:
                why = f'Start here. {info['headline']}'
            items.append(PlanItem(pos=sc.pos, rung=rung, tier=rung[2], score=sc.score, factors=sc.factors, contributions=sc.contributions, why_now=why, unlocks=unlocks, phase=phase_of[rung[2]]))
        return LearningPlan(tracks=[t for _, t in tracks], items=items, waivers=waivers, phases=sorted(phase_of.values()))
