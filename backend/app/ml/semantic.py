...
class SemanticSpace:
    ...
    def __init__(self, vectorizer: TfidfVectorizer, svd: TruncatedSVD, course_vectors: np.ndarray, tfidf_matrix=None):
    ...
    def fit(cls, catalog: Catalog) -> 'SemanticSpace':
        docs = catalog.docs
        vectorizer = TfidfVectorizer(ngram_range=(1, 2), stop_words='english', sublinear_tf=True, min_df=3, max_df=0.6, max_features=TFIDF_MAX_FEATURES)
...
def load_or_fit(catalog: Catalog, csv_path: str, cache_dir: str) -> SemanticSpace:
    ...
    if os.path.exists(path):
        try:
            ...
            if blob.get('key') == key:
                return SemanticSpace(blob['vectorizer'], blob['svd'], blob['course_vectors'], blob.get('tfidf_matrix'))
        except Exception:
            pass
    space = SemanticSpace.fit(catalog)
    try:
        joblib.dump({'key': key, 'vectorizer': space.vectorizer, 'svd': space.svd, 'course_vectors': space.course_vectors, 'tfidf_matrix': space.tfidf_matrix}, path, compress=3)
    except Exception:
        pass
    return space
