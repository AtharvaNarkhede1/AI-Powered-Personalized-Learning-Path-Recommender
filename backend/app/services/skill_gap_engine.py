...
def _verified_proficiency(db, profile_id: Optional[str], skill_id: str) -> Optional[float]:
    ...
    try:
        ...
        row = db.query(SkillProficiencyDB).filter(SkillProficiencyDB.profile_id == profile_id, SkillProficiencyDB.skill_id == skill_id).order_by(SkillProficiencyDB.updated_at.desc()).first()
...
def analyze_skill_gaps(career_id: str, profile: ProfileOnboardingRequest, db=None, profile_id: Optional[str]=None) -> SkillGapAnalysisResponse:
    ...
    for req in target_career['required_skills']:
        ...
        gaps.append(SkillGapItem(skill_id=s_id, skill_name=s_name, category=category, current_level=round(curr_level, 2), required_level=req_level, gap_delta=gap_delta, status=status, is_prerequisite=req.get('critical', False), dependencies=prereqs))
    overall_pct = round(total_acquired / max(0.1, total_required) * 100, 1)
    return SkillGapAnalysisResponse(career_id=target_career['career_id'], career_title=target_career['title'], overall_readiness_pct=min(100.0, overall_pct), gaps=gaps, prerequisite_warnings=list(set(prereq_warnings))[:3])
