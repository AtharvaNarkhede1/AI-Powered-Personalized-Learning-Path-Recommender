...
from app.models.schemas import ProfileOnboardingRequest, LearningPathResponse, Milestone, ResourceItem, NextRecommendedAction
...
def _milestone_to_db(m: Milestone) -> dict:
    return {'milestone_key': m.id, 'sequence_order': m.sequence_order, 'title': m.title, 'description': m.description, 'target_skills': m.target_skills, 'status': m.status, 'estimated_hours': m.estimated_hours, 'resources': [r.model_dump() for r in m.resources], 'project': m.project, 'assessment': m.assessment, 'youtube_extras': [r.model_dump() for r in m.youtube_extras]}

def _milestone_from_db(row: MilestoneDB) -> Milestone:
    return Milestone(id=row.milestone_key, sequence_order=row.sequence_order, title=row.title, description=row.description or '', estimated_hours=row.estimated_hours, estimated_weeks=max(1, round(row.estimated_hours / 10)), status=row.status, target_skills=row.target_skills or [], resources=[ResourceItem(**r) for r in row.resources or []], project=row.project, assessment=row.assessment, youtube_extras=[ResourceItem(**r) for r in getattr(row, 'youtube_extras', None) or []])

def get_active_path(db: Session, profile_id: str, career_id: str) -> Optional[LearningPathResponse]:
    row = db.query(LearningPathDB).filter(LearningPathDB.profile_id == profile_id, LearningPathDB.career_id == career_id, LearningPathDB.is_active == True).order_by(LearningPathDB.created_at.desc()).first()
    if not row:
        return None
    milestones = [_milestone_from_db(m) for m in row.milestones]
    next_action = NextRecommendedAction(**row.next_action) if row.next_action else NextRecommendedAction(action_type='start_course', title='Continue your path', description='Pick up where you left off.', milestone_id=milestones[0].id if milestones else 'ms_1')
    return LearningPathResponse(id=row.id, career_id=row.career_id, career_title=row.career_title, job_readiness_score=row.job_readiness_score, estimated_total_hours=row.estimated_total_hours, estimated_weeks=row.estimated_weeks, hours_per_week=max(1, round(row.estimated_total_hours / max(1, row.estimated_weeks))), milestones=milestones, next_action=next_action, what_not_to_do_warnings=row.what_not_to_do_warnings or [], track_names=getattr(row, 'track_names', None) or [])

def save_path(db: Session, profile_id: str, path: LearningPathResponse) -> None:
    existing = db.query(LearningPathDB).filter(LearningPathDB.profile_id == profile_id, LearningPathDB.career_id == path.career_id, LearningPathDB.is_active == True).first()
    if existing:
        row = existing
