"""
Progressive 5-Step Onboarding API Router.
"""
...
def search_technical_keywords(q: Optional[str] = Query(default=""), limit: int = 15):
    """Predictive autocomplete search across thousands of technical engineering keywords."""
...
def submit_onboarding_profile(user_id: str, payload: ProfileOnboardingRequest, db: Session = Depends(get_db)):
    """Saves or updates the 5-step progressive onboarding profile."""
    ...
    # Update profile fields from onboarding payload
...
def get_onboarding_profile(user_id: str, db: Session = Depends(get_db)):
    """Retrieves existing user profile or returns default mock profile."""
    profile = db.query(LearnerProfileDB).filter(LearnerProfileDB.user_id == user_id).first()
    
    if not profile:
        # Return fallback profile object
        return ProfileResponse(
            id="demo_p1",
            user_id=user_id,
