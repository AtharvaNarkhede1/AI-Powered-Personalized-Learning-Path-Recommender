...
def analyze_gaps_for_career(career_id: str, profile: ProfileOnboardingRequest, db: Session = Depends(get_db)):
    """Calculates skill gap delta, status tags (Mastered, Minor Gap, Major Gap, Missing), and prerequisite
    warnings -- preferring quiz-verified proficiency over the self-reported estimate when one exists."""
